...
namespace Tabula.Controllers
{
    ...
    public class PinController : Controller
    {
        ...
        public PinController(AppDbContext db, UserManager<Profile> userManager)
        {
            _db = db;
        ...
        public async Task<IActionResult> Index(string id)
        {
            ...
            var tupleQuery = from item in _db.Reviews where item.Pin == pin select new { Review = item, item.Profile };
        ...
        public async Task<IActionResult> CreateAsync()
        {
            ...
            var boardTitles = (from item in _db.Boards where item.Profile == currentUser select item.Title).ToArray();
        ...
        public async Task<IActionResult> Create(PinViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                Pin pin = new Pin { Title = model.Title, Image = imageData };
                pin.Id = DateTime.Now.ToString("yyyyMMddHHmmssffff");
        ...
        {
            var boardTitles = (from item in _db.Boards select item.Title).ToArray();
            ViewBag.BoardTitles = new SelectList(boardTitles);
            var pin = await _db.Pins.FirstOrDefaultAsync(p => p.Id == id);
            return View(new PinViewModel { Id = pin.Id, Title = pin.Title });
        }
        ...
        {
            var tupleQuery = from item in _db.Pins select new { Pin = item, Board = item.Board, Profile = item.Board.Profile };
